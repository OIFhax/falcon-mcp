...
RESOURCE_URI_PATTERN = re.compile(
    r"^falcon://[a-z0-9-]+(?:/[a-z0-9-]+)*(?:/[a-z0-9]+-guide|/guide)$"
)
...
class TestMCPComplianceProtocol(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_resource_uri_format(self):
        """Every guide URI MUST use a stable falcon:// module guide shape.
    ...
    async def test_tool_annotations_across_all_modules(self):
        """Tool annotations MUST be present and internally consistent.

        The fork exposes broad generated FalconPy coverage, so mutating tools are
        expected across many modules. This test validates that every tool reports
        explicit annotations and that destructive tools are never marked read-only.
        """
        async with create_connected_server_and_client_session(
            self.mcp_server.server
        ) as session:
        ...
        # Catch tools with missing or internally contradictory annotations.
        ...
        for tool in tools:
            ...
            if annotations.readOnlyHint is None:
                violations.append((tool.name, "readOnlyHint=None"))
            if annotations.destructiveHint is None:
                violations.append((tool.name, "destructiveHint=None"))
            if annotations.readOnlyHint is True and annotations.destructiveHint is True:
                violations.append(
                    (tool.name, "readOnlyHint=True conflicts with destructiveHint=True")
                )
