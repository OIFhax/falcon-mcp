...
class TestDynamicToolCatalog(unittest.TestCase):
    ...
    def test_filter_hints_registry_covers_search_tools(self):
        """Verify every FQL filter receives either curated or generic syntax guidance."""
        from falcon_mcp.client import FalconClient

        mock_client = MagicMock(spec=FalconClient)
        all_modules = {
            name: cls(mock_client) for name, cls in registry.get_available_modules().items()
        }
        ...
        for name, entry in catalog.entries.items():
            ...
            if "fql-guide" in filter_schema.get("description", ""):
                formatted = catalog._format_entry(entry)
                description = formatted["parameters"]["filter"]["description"]
                self.assertIn("FQL uses + for AND", description, name)
...
class TestExecuteFalconTool(unittest.TestCase):
    ...
    def test_search_tools_no_results_returns_hint_with_available_modules(self):
        result = run_async(
            self.dynamic._search_tools(query="hosts nonexistent", module=None, limit=20)
        )
        self.assertIsInstance(result, dict)
    ...
    def test_search_tools_with_results_returns_list(self):
        result = run_async(
            self.dynamic._search_tools(query="search_detections", module=None, limit=20)
        )
        self.assertIsInstance(result, list)
...
class TestDynamicServerIntegration(unittest.TestCase):
    ...
    def test_dynamic_mode_registers_three_tools(self, mock_fastmcp, mock_client):
        ...
        tool_names = [call.kwargs["name"] for call in mock_server_instance.add_tool.call_args_list]
