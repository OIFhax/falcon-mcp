...
class RTRModule(BaseModule):
    ...
    def register_resources(self, server: FastMCP) -> None:
        ...
        self._add_resource(
            ...
            TextResource(
                ...
                description="Explains how to summarize RTR session activity.",
        ...
        self._add_resource(
            ...
            TextResource(
                ...
                description="Provides a safe read-only RTR endpoint investigation workflow.",
    ...
    def search_rtr_admin_scripts(
        ...
        if not script_ids:
            if filter:
                return self._format_fql_error_response(
                    [], filter, SEARCH_RTR_ADMIN_FQL_DOCUMENTATION
                )
            return []
    ...
    def batch_init_rtr_sessions(
        ...
        timeout_duration: str | None = Field(
            default=None, description="Request timeout duration string."
        ),
        host_timeout_duration: str | None = Field(
            default=None, description="Per-host timeout duration string."
        ),
        body: dict[str, Any] | None = Field(
            default=None,
            description="Full request body override. If provided, convenience fields are ignored.",
    ...
    def search_rtr_falcon_scripts(
        ...
        limit: int = Field(
            default=10, ge=1, le=5000, description="Maximum number of Falcon script IDs to return."
        ),
    ...
    def search_sessions(
        ...
        filter: str | None = Field(
            default=None,
            description=(
                "FQL filter expression. See `falcon://rtr/sessions/search/fql-guide` for syntax."
            ),
    ...
    def execute_read_only_command(
        ...
        command_string: str | None = Field(
            default=None,
            description=(
                "Optional full command line to execute. Example: `cat C:\\Windows\\win.ini`."
            ),
        ),
        persist: bool = Field(
            default=False,
    ...
    def run_read_only_command_and_wait(
        ...
        while True:
            ...
            if isinstance(status_result, list):
                status_chunks.extend(chunk for chunk in status_result if isinstance(chunk, dict))
