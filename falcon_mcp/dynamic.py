...
class ToolEntry:
    ...
    def __post_init__(self) -> None:
        ...
        parameter_text = " ".join(
            f"{name} {schema.get('description', '')}" for name, schema in properties.items()
        )
        filter_hint = FILTER_HINTS.get(self.tool.name, "")
        self.search_corpus = (
            f"{self.tool.name} {self.tool.description or ''} {self.module} "
            f"{parameter_text} {filter_hint}"
        ).lower()
        ...
        try:
            ...
            if "validation" in error_type.lower() or "valid" in str(e).lower():
                return {
                    ...
                    "tool": tool_name,
                    "expected_parameters": self.catalog.summarize_parameters(entry.tool.parameters),
                }
